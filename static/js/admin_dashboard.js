...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function renderRepoResults(data) {
    ...
    if (!data.items || data.items.length === 0) {
      ...
      const info = document.getElementById('repo-page-info'); if (info) info.textContent = '';
      return;
    }
    ...
    const total = data.total || 0; const page = data.page || repoState.page; const size = data.page_size || repoState.pageSize;
    const info = document.getElementById('repo-page-info');
    if (info) info.textContent = `第 ${page} 页 / 共 ${Math.ceil(total / size)} 页，${total} 条`;
    const prev = document.getElementById('repo-prev'); const next = document.getElementById('repo-next');
  ...
  if (repoPrev) repoPrev.addEventListener('click', () => { if (repoState.page>1) { repoState.page--; loadRepos(); } });
  if (repoNext) repoNext.addEventListener('click', () => { repoState.page++; loadRepos(); });
  ...
  if (addRepoBtn) addRepoBtn.addEventListener('click', () => {
    ...
    openModal('添加仓库', `
      ...
      <div class="form-group">
        ...
        <button type="button" id="modal-generate-summary" class="btn btn-secondary btn-sm" style="margin-top:0.5rem;">
  ...
  async function deleteCategory(c) {
    ...
    if (!res.ok) { const j = await res.json().catch(()=>({detail:'失败'})); alert(j.detail || '删除失败'); return; }
    loadCategories(); populateCategoryTrees();
  }
  ...
  async function deleteRepo(r) {
    ...
    if (!res.ok) { const j = await res.json().catch(()=>({detail:'删除失败'})); alert(j.detail || '删除失败'); return; }
    loadRepos();
  }
